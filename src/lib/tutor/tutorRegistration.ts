...
export const tutorRegistrationApi = {
  ...
  saveCertificationStep: async (tutorId: number, data: any) => {
      const formData = new FormData();
      formData.append('tutorId', tutorId.toString());
      formData.append('hasNoCertificate', data.hasNoCertificate ? 'true' : 'false');
      ...
      if (data.certifications && data.certifications.length > 0) {
        data.certifications.forEach((cert: any, index: number) => {
          ...
          formData.append(`certifications[${index}][yearsFrom]`, cert.yearsFrom || '');
          formData.append(`certifications[${index}][yearsTo]`, cert.yearsTo || '');
          if (cert.file) {
  ...
  saveEducationStep: async (tutorId: number, data: any) => {
      ...
      if (data.education && data.education.length > 0) {
        data.education.forEach((edu: any, index: number) => {
          ...
          formData.append(`education[${index}][degreeType]`, edu.degreeType || '');
          formData.append(`education[${index}][specialization]`, edu.specialization || '');
...
saveAvailabilityStep: async (tutorId: number, data: any) => {
    ...
    const availabilityArray = Object.keys(data.availability || {})
      .filter(day => {
        ...
        return slots && slots.length > 0 && slots.some((slot: any) => slot.from && slot.to);
      })
      .map(day => ({
        day: day,
