...
import TopBar from "@/components/student_details/TopBar";
import Link from "next/link";
import { authService } from "@/lib/auth/authService";
import { useEffect, useState } from "react";
import ClassHeaderCard from "@/components/student_details/ClassHeaderCard";
        ...
        <div className="max-w-5xl mx-auto px-6 flex justify-center gap-8">
          {tabs.map(tab => {
            ...
            return (
              <Link
                key={tab}
                href={`/student-portal/classes/${id}/${tab}`}
              ...
              >
                {tab === "lessons" && "Lessons"}
                {tab === "assignments" && "Homework"}
                {tab === "people" && "Classmates"}
                {tab === "grades" && "My Grades"}
